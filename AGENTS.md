...
- **PozKes Galerisi ve Sosyal Paylaşım Mimarisi:** PozKes akışı `topic.slug = "pozkes-galeri"` başlığına bağlıdır ve `/yeni` başlık resimlerinden yalıtılmıştır. PozKes kartlarında WhatsApp, X ve Link Kopyala paylaşım menüsü yer alır. `#entry-ID` çapa (anchor) bağlantıları SEO dostu deep-linking ile doğrudan ilgili karta kilitlenir.
